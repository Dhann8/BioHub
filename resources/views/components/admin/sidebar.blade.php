<aside id="sidebar" class="w-64 h-screen sticky top-0 bg-[#0f2817] flex flex-col flex-shrink-0 z-20">
  ...
  <div class="flex items-center gap-3 px-5 py-6 border-b border-white/10">
    <div class="w-9 h-9 bg-[#D97706] rounded-xl flex items-center justify-center flex-shrink-0">
      <i class="fa-solid fa-leaf text-white text-base"></i>
    </div>
    <span class="text-white font-bold text-lg tracking-wide">BiodivAdmin</span>
    <button id="collapse-btn" class="ml-auto text-white/40 hover:text-white/80 transition-colors">
  ...
  <nav class="flex-1 px-3 pt-5 space-y-1 overflow-y-auto scrollbar-thin">
    ...
    <a href="{{ route('admin.dashboard') }}" class="sidebar-item flex items-center gap-3 px-3 py-2.5 rounded-lg cursor-pointer {{ request()->routeIs('admin.dashboard') ? 'sidebar-active' : 'text-white/70' }}">
    ...
    <div class="space-y-1">
      <button type="button" 
        ...
        <span class="ml-auto text-white/40 text-xs transition-transform duration-200" id="fauna-menu-arrow">
          <i class="fa-solid fa-chevron-down text-[10px] {{ request()->routeIs('admin.fauna.*') || request()->routeIs('admin.fauna-locations.*') || request()->routeIs('admin.fauna-details.*') ? 'rotate-180' : '' }}"></i>
        </span>
      </button>

      <div id="fauna-submenu" class="pl-7 space-y-1 {{ request()->routeIs('admin.fauna.*') || request()->routeIs('admin.fauna-locations.*') || request()->routeIs('admin.fauna-details.*') ? '' : 'hidden' }}">
    ...
    <div class="space-y-1">
      <button type="button" 
        ...
        <span class="ml-auto text-white/40 text-xs transition-transform duration-200" id="herbal-menu-arrow">
          <i class="fa-solid fa-chevron-down text-[10px] {{ request()->routeIs('admin.herbal.*') || request()->routeIs('admin.herbal-details.*') ? 'rotate-180' : '' }}"></i>
        </span>
      </button>

      <div id="herbal-submenu" class="pl-7 space-y-1 {{ request()->routeIs('admin.herbal.*') || request()->routeIs('admin.herbal-details.*') ? '' : 'hidden' }}">
    ...
    <a href="{{ route('admin.crowdsourcing.index') }}" class="sidebar-item flex items-center gap-3 px-3 py-2.5 rounded-lg cursor-pointer {{ request()->routeIs('admin.crowdsourcing.*') ? 'sidebar-active' : 'text-white/70' }}">
      ...
      @if($pendingCrowdsource > 0)
        <span class="ml-auto bg-[#D97706] text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">{{ $pendingCrowdsource }}</span>
      @endif
    </a>

    <a href="{{ route('map') }}" class="sidebar-item flex items-center gap-3 px-3 py-2.5 rounded-lg cursor-pointer {{ request()->routeIs('map') ? 'sidebar-active' : 'text-white/70' }}">
      <i class="fa-solid fa-map-location-dot w-4 text-center"></i>
      <span class="text-sm font-medium">Web GIS Spatial Data</span>
    </a>

    <a href="{{ route('admin.users.index') }}" class="sidebar-item flex items-center gap-3 px-3 py-2.5 rounded-lg cursor-pointer {{ request()->routeIs('admin.users.*') ? 'sidebar-active' : 'text-white/70' }}">
  ...
  <div class="px-4 py-4 border-t border-white/10">
    ...
    <form action="{{ route('logout') }}" method="POST">
      @csrf
      <button type="submit" class="w-full flex items-center justify-center gap-2 bg-white/10 hover:bg-white/15 text-white/80 text-xs font-medium py-2 rounded-lg transition-colors">
